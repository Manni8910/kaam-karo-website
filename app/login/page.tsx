      ...
      <div className="bg-white rounded-2xl shadow-sm p-8 w-full max-w-md">
        ...
        {method === "phone" ? (
          step === "input" ? (
              ...
              <div className="mb-4">
                ...
                <div className="flex">
                  ...
                  <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value.replace(/\D/g, "").slice(0, 10))}
                    placeholder="9876543210" className="flex-1 px-4 py-3 border border-gray-300 rounded-r-lg focus:outline-none focus:border-[#1B3FAB]" />
                </div>
              </div>
            ...
            <>
              <p className="text-sm text-gray-500 mb-4">OTP sent to +91 {phone} <button onClick={() => setStep("input")} className="text-[#1B3FAB] underline">Change</button></p>
          ...
          <>
            ...
            <div className="mb-2">
              ...
              <div className="relative">
                <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)}
                  placeholder="••••••••" className="w-full px-4 py-3 pr-12 border border-gray-300 rounded-xl focus:outline-none focus:border-[#1B3FAB]" />
            ...
            <div className="flex justify-end mb-6">
              <Link href="/forgot-password" className="text-sm text-[#1B3FAB] hover:underline">
                Forgot password?
              </Link>
            </div>
            {error && <p className="text-red-500 text-sm mb-4">{error}</p>}
        ...
        <p className="text-center text-sm text-gray-500 mt-6">
          Don't have an account? <Link href="/register" className="text-[#1B3FAB] font-medium">Register</Link>
        </p>
      </div>
    </div>
  );
